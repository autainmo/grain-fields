# Style Core

## 1. 风格核心命题

真实主体保持稳定，背景按画面实际内容划分为 2–3 个语义完整区域，并在每个方案中选择一整块大区域进行强烈的大颗粒转译，使主体、文字、干净区域与颗粒区域形成清晰而有呼吸感的视觉层级。

## 2. 最终视觉身份

本 Skill 的视觉身份不是某一种固定“复古”“海报”或“胶片”风格，而是一套稳定的重构结构：

- 主体可信、可辨、结构稳定；
- 背景按内容而不是几何位置分区；
- 每张图都有一个大面积、整块、粗颗粒视觉区域；
- 主体以醒目的外轮廓与背景分离；
- 四个方案的配色类型都不同；方案 1–3 允许重新设计配色并形成彼此不同的视觉赛道，且不得与方案 4 的原图配色保留方向混同；
- 四个方案的文本类型都不同，并分别结合各自方案内容独立成立；
- 方案 4 保留原图色彩与构图；
- 文字若存在，必须进入版面逻辑而不是后贴标签。

## 3. Style Invariants｜风格不变量

以下规则无论主体是人物、动物、建筑、街道、车辆、植物、山川、水面、器物或其他对象，都必须成立：

1. **主体辨识稳定**：主要主体的身份、类别、外形、结构、比例、姿态或关键辨识细节不得因重构而失真。除用户明确要求外，主体本身不做明显重构，应尽量保持原图形，只允许对影调与颜色参数进行轻微优化微调。
2. **内容分区**：背景必须依据实际景物、空间层次、材质或连续结构归纳为 2–3 个主要区域；不得机械按上下左右切割。
3. **完整对象不拆分**：同一个完整主体或同一连续景物原则上只能属于一个内容区域，不得为了分区效果强行切断。
4. **大颗粒必现**：四个默认方案中，每张图都必须存在一个面积明显、连续完整、颗粒单元明显偏大的核心颗粒区域；效果不得退化为轻微滤镜，并应在不破坏主体与场景关系的前提下让颗粒尽可能明显。
5. **主体轮廓分离**：主要视觉主体沿真实外缘增加清晰轮廓线；用户无明确要求时严格默认使用中粗纯白色单线，不因背景颜色自动改为其他描边；只有用户明确指定其他描边方式时才按用户要求调整。
6. **四方案差异**：方案 1–3 必须属于明显不同的视觉配色方向；方案 4 必须保留原图主配色与构图；四个方案的配色类型不得重复。
7. **文字服从画面**：文字只在用户提供或自动生成时出现，并根据留白、主体位置和颗粒区位置合理排布；事实信息不得编造；四个方案的文本类型不得重复。
8. **随机但适配**：默认选择配色类型与文本类型时，可在适配候选中随机抽选，但随机结果仍须服务主题、主体与画面质量。
9. **粗净对比**：主要颗粒区与其余较干净区域必须形成可观察的粗—净、密—疏差异，使画面有层次与呼吸感。
10. **成图清晰**：每张重构图整体必须清晰；颗粒区可以概括局部细节，但主体、主要结构、区域边界与整体画面不得发虚、模糊或呈现低清感。
11. **主体—颗粒呼吸**：核心颗粒区与主体之间必须保留视觉缓冲与分离感，不让高密度颗粒紧贴、吞没或压迫主体轮廓。
12. **背景配色干净**：背景重构后的综合色必须协调、克制、有秩序；不得因多色叠加、灰化混色或过度纹理造成杂乱、浑浊、脏灰或脏版感。

## 4. Source Role｜原图角色

原图在本 Skill 中同时承担以下角色：

- **S2 主体摄影保留**：主要主体是最终作品中最可信的来源，优先保留其真实外形、姿态、比例、材质与识别特征。
- **S3 场景语义地图**：背景中的海、天、地面、建筑、植被、墙体、道路、水面、岩石等实际内容决定分区边界与空间关系。
- **S4 质感转译底稿**：选定的核心背景区域保留其大体结构与明暗关系，但细节被重新组织为大颗粒 / 大网点 / 粗半色调等视觉结构。
- **S1 原图整体保留（仅方案 4）**：方案 4 额外把原图主配色、构图、主体位置、比例和景物关系作为强锁定条件。

## 5. 原图与重构关系

### 必须保留

- 主体身份或类别；
- 关键外形、结构、比例、姿态；
- 主体与环境的基本空间关系；
- 能说明场景的关键环境元素；
- 用户明确要求保留的文字、事实、构图或颜色。

### 必须发生变化

- 背景被归纳为 2–3 个内容区域；
- 每个方案至少一个完整背景区域被重构为显著大颗粒区域；
- 主要主体获得轮廓线；
- 方案 1–3 的背景色彩关系被重新设计；
- 文字若存在，需要进入版式关系。

### 仅方案 4 不得变化

- 原图整体主配色；
- 原构图；
- 主体位置与整体比例关系；
- 原场景中主要景物的相对位置。

## 6. 标志性视觉特征

- 一个面积大且连续完整的粗颗粒区域；
- 颗粒在正常观看尺寸下可明显辨认，不是微噪声；
- 背景分区来自内容本身，边界有语义理由；
- 主要主体具有清晰外轮廓；
- 大颗粒区与干净区存在明显材质反差；
- 三个重新配色方案具有真正不同的色彩叙事；
- 第四方案体现“原图色彩 + 同一重构语言”。

## 7. 信息密度

- 画面应优先保持一个明确主视觉焦点。
- 背景分区数量固定为 2–3 个主要区域，避免切碎。
- 核心颗粒区只选择一个主要区域，避免多个区域都做同强度颗粒导致拥挤。
- 颗粒效果必须作用于整个选定分区，而不是只在该分区内做局部覆盖。
- 文字信息以必要层级为限；用户提供什么就合理展示什么，不强制补齐所有字段。
- 自动生成文字时，优先短标题、短副标题、短正文或少量信息字段，而不是大段说明。

## 8. 留白逻辑

- 留白不是空缺，而是用于分隔主体、文字和颗粒区的视觉缓冲。
- 主要文字优先落在相对干净、可读性高的区域。
- 核心颗粒区应尽量避开主要文字。
- 若主体已经占据大面积画面，颗粒区应选择不破坏主体轮廓的连续背景区。
- 核心颗粒区应尽可能优先选择与主要主体直接相邻、紧挨主体的完整背景板块；只有相邻区域明显不满足面积、连续性、主体安全或当前方案适配时，才改选其他完整背景区域。若背景分区呈左中右、上中下或中心被外围包围的中间结构，则优先选择中间分区。
- 若背景很满，应通过降低其他区域纹理强度制造呼吸，而不是继续添加装饰。

## 9. 风格强度

- 主体保留强度：高；默认仅对主体影调与颜色参数做轻微优化微调，不做明显重构。
- 背景重构强度：中高至高。
- 核心颗粒区强度：高，必须第一眼可见，并在满足主体安全与场景逻辑的范围内尽可能明显。
- 其他背景区纹理强度：低至中。
- 文字装饰强度：随画面而定，但不得压过主体和颗粒结构。
- 方案 1–3 的配色变化强度：中高，可脱离原图综合色。
- 方案 4 的配色变化强度：零，仅允许质感、对比、清晰度等不改变综合色关系的优化。

## 10. Hard Avoids｜绝对禁止项

- 机械二等分、三等分画面来冒充内容分区；
- 将同一人物、同一建筑、同一连续山体、同一车辆等硬拆成多个区域；
- 颗粒区域过小、只在边角出现、只做成淡噪点；
- 所有背景区都同样粗糙，失去粗净反差；
- 大颗粒覆盖主文字造成阅读困难；
- 主体轮廓线消失、过细到不可见或粗到吞没主体细节；
- 方案 1–3 只是同一配色的轻微变化；
- 方案 4 改变原图综合色、主要构图或主体位置；
- 为制造艺术感而改掉人物身份、动物种类、建筑结构、器物形态等核心特征；
- 自动编造用户未给出的事实信息；
- 以“复古”“高级”“海报”等模糊标签代替可执行规则。
