...
人物、动物、建筑、山川、植物、车辆、器物等都可以作为主要主体。主体保留关键外形、结构、比例、姿态和识别信息，并沿真实外缘加清晰轮廓。背景颗粒化使用自然、适中、克制的颗粒语言；用户可以指定颗粒预设，也可以让 Skill 自动匹配。
...
以下为仓库当前颗粒、配色、文本与比例预设索引。

### 颗粒预设
> 每个预设的适配逻辑与执行规则见 [`references/presets-grain.md`](references/presets-grain.md)。
...
> 内置颗粒库明确排除：大方块、大像素、拼豆、巨大马赛克砖、梦幻方块、玻璃方块、巨型低多边形、玩具 / 糖果式颗粒。

### 配色预设
> 每个预设的简洁介绍与选型依据见 [`references/presets-color.md`](references/presets-color.md)。自动模式优先使用更克制的方向；用户明确指定其他预设时仍可使用，但会限制颜色数量与装饰强度。
